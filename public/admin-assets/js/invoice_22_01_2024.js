var AppsScriptLink = "AKfycbztg3I_NSUx4C998igeajyklH4zUJ3bE7bjEzx3uWN_r6CC5XNgNsmko8lPpCvpyRUb9g"; // exaltedsol04 latest

//var AppsScriptLink = "AKfycbxfiEMQkSPt380K-Ggc4ab3yXiQ4rF7uqkyfj_CIp9ggiLB_dmfkZJoC3XxyWN3-gCXlA"; // exaltedsol04 previous
...
$(document).ready(function () {
	...
	var URL6mmMat = '/getMaterial6mmVal';
	$.ajax({
        url: URL6mmMat,
		type: "get",
		data: { _token: "{{ csrf_token() }}"},
	...
	var URL18mmShu = '/getShutterMaterial18mmVal';
	$.ajax({
        url: URL18mmShu,
		type: "get",
		data: { _token: "{{ csrf_token() }}"},
	...
	var URLExpoPrice = '/getExposidePrice';
	$.ajax({
        url: URLExpoPrice,
		type: "get",
		data: { _token: "{{ csrf_token() }}"},
    ...
    //------ for material---------------------
	//"{{ route('admin.materialUrl') }}",
	var urlMaterial = '/materialUrl';
	$.ajax({
		headers: {
...
function searchCode(arr)
{
	...
	//alert(selectedhandeltype.id);
		...
		var A1 = LengthDeep(searchLength,searchDeep,searchQuantity);
		var A2 = DeepWidth(searchWidth,searchDeep,searchQuantity);
		var A3 = LengthWidth(searchWidth,searchLength,searchQuantity);
...
function handleSubmit() {
	...
	else
	{
		  ...
		  //alert(subcategoryID);alert(cabinetTypeID);
			...
			var A1 = LengthDeep(length,deep,qty);
			var A2 = DeepWidth(width,deep,qty);
			var A3 = LengthWidth(width,length,qty);
...
function handleSearchSubmit() {
	...
	else
	{
		...
		var hardWareAmt = 500;
		if(selectedexposideText == "Left Side Expo" || selectedexposideText == "Right Side Expo")
		{
			//var expoValue   =  parseFloat(A1*0.5*60) + parseFloat(hardWareAmt);
			expoValue  		=  (A1*0.5*60) + hardWareAmt;
		}
		else if(selectedexposideText == "Both side Expo")
		{
			//console.log(selectedexposideText);
			var expoValue  =  parseFloat(A1*60) + parseFloat(hardWareAmt);
		}
		else if(selectedexposideText == "Without Expo")
		{
			var expoValue  =  hardWareAmt;
...
function ResetTable()
{
	...
	function LengthDeep(length,deep,qty)  // A1
	{
		var result = (2 * length * deep * qty) / 92903.4;
		return result.toFixed(2);
	}
	function DeepWidth(width,deep,qty) // A2
	{
		var result = (2 * width * deep * qty) / 92903.4;
		return result.toFixed(2);
	}
	function LengthWidth(width,length,qty) // A3
	{
		var result = (1 * width * length * qty) / 92903.4;
		return result.toFixed(2);
	}
