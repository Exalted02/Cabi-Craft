...
class CmsController extends Controller
{
    ...
    public function manage_cms_process(Request $request)
    {
		...
		//$isTrending 	= $request->has('is_trending') ? 1 : 0;
		if($request->post('id')>0){
			$model = Cms_pages::find($request->post('id'));
	...
	public function cms_apply($id='')
	{
		$cmsApply = Cms_form_submissions::query()->with('cms_submission')->where('cms_id', $id)->where('status', '!=', 2)->get();
		$result['data']=$cmsApply;
        return view('admin.cms.cmsapply',$result);
	}
	public function cms_apply_view(Request $request)
	{
		$select_val = $arr = Cms_form_submissions::query()->with('cms_submission')->where('id', $request->id)->first();
		//echo "<pre>";print_r($select_val); die;
		$var = '';
		$var .= '<div class="cms_body"><h6><b>Page Name :</b>'.$select_val->cms_submission->cms_page_name.'</h6></div><div class="cms_body"><h6><b>Sender Name :</b>'.$select_val->name.'</h6></div><div class="cms_body"><h6><b>Sender Email :</b>'.$select_val->email.'</h6></div><div class="cms_body"><h6><b>Sender Contact No. :</b>'.$select_val->phone.'</h6></div>
				<div class="cms_body"><h6><b>Page Content :</b>'.$select_val->comment.'</h6></div>';		
		return response()->json(['html'=>$var]);
	}
}
